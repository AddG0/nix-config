...
# Current system double (e.g. x86_64-linux); used for flake eval throughout.
...
resolve_pkg_file() {
  local pkg="$1" pos rel
  pos="$(nix eval --raw "$FLAKE_DIR#packages.$SYSTEM.\"$pkg\".meta.position" 2>/dev/null)" || return 1
  [[ -z $pos ]] && return 1
  pos="${pos%:*}" # strip trailing :LINE
  if [[ $pos == "$FLAKE_DIR"/* ]]; then
    printf '%s\n' "$pos"
  elif [[ $pos == *-source/* ]]; then
    rel="${pos#*-source/}" # strip the /nix/store/<hash>-source/ prefix
    printf '%s\n' "$FLAKE_DIR/$rel"
  else
    return 1
  fi
...
update_one() {
  ...
  # Back up the package's source file so a failed update can be rolled back.
  # nix-update edits the version string before fetching the new source, so a
  # fetch/build failure (common under high parallelism) would otherwise leave
  # the file with a bumped version but a stale hash, breaking the build.
  local pkg_file backup_file
  pkg_file="$(resolve_pkg_file "$pkg_name" 2>/dev/null || true)"
  backup_file="$LOG_DIR/snapshots/$pkg_name.filebackup"
...

  {
    ...
    # Roll back any partial edit nix-update left behind so a failed update
    # never leaves the working tree with an inconsistent (version-bumped,
    # stale-hash) derivation.
    local reverted=""
    if [[ -n $pkg_file && -f $backup_file ]] && ! diff -q "$backup_file" "$pkg_file" >/dev/null 2>&1; then
      cp -p "$backup_file" "$pkg_file"
      reverted=" (reverted partial changes)"
    fi
    local err
    err=$(extract_error_excerpt "$log_file")
    if [[ -n $err ]]; then
      log_error "$pkg_name - FAILED: $err$reverted"
    else
      log_error "$pkg_name - FAILED${reverted} (see .update-logs/failed/$pkg_name.log)"
    fi
    return 1
  }
}
...
# `set -e` would abort before the failure-summary below runs. Per-package
# failures are expected and reported in the summary, so don't let one masked
# failure kill the whole run.
printf '%s\n' "${to_update[@]}" | parallel --halt never --line-buffer -j "$JOBS" update_one {} || true
