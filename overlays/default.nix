...
{inputs, ...}: let
  ...
  darwinModifications = _final: prev:
    ...
    then {
      ...
      # $out/<rootDir>/bin/* (inside the .app bundle) but misses $out/bin/*
      # where the convenience launcher lives, leaving a disallowed reference
      # to the unwrapped IDE.
      jetbrains =
        prev.jetbrains
        // {
          plugins =
  ...
  in {
    ...
    vscode-marketplace =
      ...
      // {
        # Fix shell-format 7.2.8 missing @one-ini/wasm runtime.
        # Without one_ini_bg.wasm in dist/, the extension fails to register as a
        # formatter and VSCode reports "There is no formatter for shellscript".
    ...
    # wpaperd 1.3.0 — nixpkgs still ships 1.2.2 which is missing the
    # `wpaperctl set <file> <output>` subcommand the wallpaper-picker
    # needs. cargoHash on a `finalAttrs:`-style buildRustPackage doesn't
    # propagate via overrideAttrs; replace cargoDeps directly instead.
    # Drop this override once nixpkgs catches up.
    ...
    # Boxflat: silence the "needs to update udev rules" alert. The rule is
    # installed declaratively via services.udev.packages = [pkgs.boxflat],
    # so boxflat's in-app rules-version check (which lives in mutable
    # ~/.config/boxflat/settings.yml) is a false positive on NixOS.
    # --replace-fail intentionally — if upstream changes this line, the
    # build will break loudly and we'll know to revisit.
  ...
  # hyprland and hyprlandPlugins.hy3 out of sync (e.g. hyprland 0.55.2 vs
  # hy3 0.55.0), so the plugin refuses to load with a version-mismatch
  # error. hy3 builds against the exact Hyprland commit it pins, so sourcing
  # pkgs.hyprland from hy3's own hyprland input — rather than the other way
  # round — guarantees the two always match.
  hyprland-hy3 = final: prev: let
    inherit (final.stdenv.hostPlatform) system;
  in
    prev.lib.optionalAttrs prev.stdenv.isLinux {
