...
# ==============================================================================
{
  ...
  # Plugin dirs case block (generated at Nix eval time)
  pluginDirsCase = lib.concatStringsSep "\n" (lib.mapAttrsToList (
      name: profile: let
        resolved = mergeWithBase name profile;
  ...
  wrapperScript = pkgs.writeShellScriptBin "claude" ''
    ...
    # Plugin dirs (generated at build time from profile configs)
    PLUGIN_ARGS=()
    case "$PROFILE" in
    ${pluginDirsCase}
      *) ;;
    ...
    # LSP plugin dir (generated by home-manager)
...
in {
  ...
  config = lib.mkIf cfg.enable {
    programs.claude-code-profiles.resolved =
      lib.mapAttrs mergeWithBase cfg.profiles;
