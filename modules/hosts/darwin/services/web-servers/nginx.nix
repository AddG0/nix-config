...
in {
  ...
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = [
      cfg.package
      # Add Nginx service management scripts
      (pkgs.writeShellScriptBin "nginx-start" ''
        echo "Starting Nginx service..."
        sudo launchctl load /Library/LaunchDaemons/org.nixos.nginx.plist 2>/dev/null || echo "Service already loaded"
        echo "Nginx service started"
      '')
      (pkgs.writeShellScriptBin "nginx-stop" ''
        echo "Stopping Nginx service..."
        sudo launchctl unload /Library/LaunchDaemons/org.nixos.nginx.plist 2>/dev/null || echo "Service not loaded"
        echo "Nginx service stopped"
      '')
      (pkgs.writeShellScriptBin "nginx-restart" ''
      ...
      '')
      (pkgs.writeShellScriptBin "nginx-reload" ''
      ...
      '')
      (pkgs.writeShellScriptBin "nginx-status" ''
        if sudo launchctl list | grep -q "org.nixos.nginx"; then
          PID=$(sudo launchctl list | grep "org.nixos.nginx" | awk '{print $1}')
          if [[ "$PID" =~ ^[0-9]+$ ]]; then
            echo "Nginx service is running (PID: $PID)"
            echo "Configuration test:"
            ${execCommand} -t 2>/dev/null && echo "✓ Configuration is valid" || echo "✗ Configuration has errors"
          else
            echo "Nginx service is loaded but not running (status: $PID)"
          fi
        else
          echo "Nginx service is not loaded"
        fi
      '')
      (pkgs.writeShellScriptBin "nginx-logs" ''
        echo "=== Nginx Access Log ==="
        if [[ -f "/opt/nginx/logs/access.log" ]]; then
          tail -n 20 "/opt/nginx/logs/access.log"
        else
          echo "Access log not found"
        fi
        echo
        echo "=== Nginx Error Log ==="
        if [[ -f "/opt/nginx/logs/error.log" ]]; then
          tail -n 20 "/opt/nginx/logs/error.log"
        else
    ...
    # Create nginx directories during system activation (runs as root)
    system.activationScripts.extraActivation.text = lib.mkAfter ''
      if [[ ! -d "/opt/nginx" ]]; then
        ...
        chown -R ${cfg.user}:${cfg.group} "/opt/nginx"
        chmod -R 755 "/opt/nginx"
        echo "Nginx directories created successfully"
      fi
    '';

    # Note: ACME certificate configuration removed for Darwin - configure SSL certificates manually
