...
in {
  ...
  options.services.grafana = {
    ...
    user = mkOption {
      ...
      default = "_grafana";
      description = ''
        User account under which Grafana runs.
      '';
    };
    ...
    group = mkOption {
      ...
      default = "_grafana";
      description = ''
        Group account under which Grafana runs.
      '';
    };
  ...
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = [
      cfg.package
      # Add Grafana service management scripts
      (pkgs.writeShellScriptBin "grafana-start" ''
        echo "Starting Grafana service..."
        sudo launchctl load /Library/LaunchDaemons/org.nixos.grafana.plist 2>/dev/null || echo "Service already loaded"
        echo "Grafana service started"
      '')
      (pkgs.writeShellScriptBin "grafana-stop" ''
        echo "Stopping Grafana service..."
        sudo launchctl unload /Library/LaunchDaemons/org.nixos.grafana.plist 2>/dev/null || echo "Service not loaded"
        echo "Grafana service stopped"
      '')
      (pkgs.writeShellScriptBin "grafana-restart" ''
        echo "Restarting Grafana service..."
        sudo launchctl unload /Library/LaunchDaemons/org.nixos.grafana.plist 2>/dev/null || echo "Service not loaded"
        sleep 2
        sudo launchctl load /Library/LaunchDaemons/org.nixos.grafana.plist
        echo "Grafana service restarted"
      '')
      (pkgs.writeShellScriptBin "grafana-status" ''
        if sudo launchctl list | grep -q "org.nixos.grafana"; then
          PID=$(sudo launchctl list | grep "org.nixos.grafana" | awk '{print $1}')
          if [[ "$PID" =~ ^[0-9]+$ ]]; then
            echo "Grafana service is running (PID: $PID)"
            echo "Web UI: http://${cfg.settings.server.http_addr}:${toString cfg.settings.server.http_port}"
          else
            echo "Grafana service is loaded but not running (status: $PID)"
          fi
        else
          echo "Grafana service is not loaded"
        fi
      '')
      (pkgs.writeShellScriptBin "grafana-logs" ''
        echo "=== Grafana Logs ==="
        if [[ -f "${cfg.dataDir}/grafana.log" ]]; then
          tail -n 50 "${cfg.dataDir}/grafana.log"
        else
          echo "Log file not found at ${cfg.dataDir}/grafana.log"
        fi
      '')
    ];
    ...
    assertions = [
      ...
      }
      {
        ...
        message = "Grafana data directory should be in a system location like /opt, /usr/local, or /var. Current path: ${cfg.dataDir}";
      }
      {
        assertion = cfg.settings.server.http_port >= 1024;
        message = "Grafana port must be >= 1024 for non-root users. Current port: ${toString cfg.settings.server.http_port}";
      }
    ];
    ...
    launchd.daemons.grafana = let
      grafanaScript = pkgs.writeShellScript "grafana-daemon" ''
        set -euo pipefail
        ...
        cleanup() {
          if [[ -n "''${GRAFANA_PID:-}" ]] && kill -0 "$GRAFANA_PID" 2>/dev/null; then
            log "Shutting down Grafana (PID: $GRAFANA_PID)"
            kill "$GRAFANA_PID"
        ...
        if [[ ! -d "${cfg.dataDir}" ]]; then
          log "ERROR: Grafana data directory ${cfg.dataDir} does not exist!"
          exit 1
        fi

        # Create symlinks for static content
        ln -sf ${cfg.package}/share/grafana/conf ${cfg.dataDir}/
        ln -sf ${cfg.package}/share/grafana/tools ${cfg.dataDir}/

        # Start Grafana in background
        log "Starting Grafana daemon..."
        log "Command: ${cfg.package}/bin/grafana server -homepath ${cfg.dataDir} -config ${configFile}"
        ${cfg.package}/bin/grafana server -homepath ${cfg.dataDir} -config ${configFile} &
        GRAFANA_PID=$!
        log "Grafana daemon started with PID: $GRAFANA_PID"

        # Wait for Grafana to be ready
        log "Waiting for Grafana to be ready..."
        TIMEOUT=30
        COUNTER=0
        while ! curl -s "http://${cfg.settings.server.http_addr}:${toString cfg.settings.server.http_port}/api/health" > /dev/null 2>&1; do
          if ! kill -0 $GRAFANA_PID 2>/dev/null; then
            log "ERROR: Grafana daemon (PID: $GRAFANA_PID) exited unexpectedly"
            exit 1
          fi
          if [ $COUNTER -ge $TIMEOUT ]; then
            log "ERROR: Grafana failed to start within $TIMEOUT seconds"
            exit 1
          fi
          sleep 1
          COUNTER=$((COUNTER + 1))
        done
        log "Grafana is ready at http://${cfg.settings.server.http_addr}:${toString cfg.settings.server.http_port}"

        # Wait for Grafana daemon to finish
        log "Grafana service is running. Waiting for shutdown signal..."
        wait $GRAFANA_PID
      '';
    in {
      script = "${grafanaScript}";
      serviceConfig = {
        KeepAlive = true;
        RunAtLoad = true;
        ProcessType = "Background";
        StandardOutPath = "${cfg.dataDir}/grafana.log";
        StandardErrorPath = "${cfg.dataDir}/grafana.error.log";
        UserName = cfg.user;
        GroupName = cfg.group;
