...
in {
  imports = [
    ...
    (
      ...
      "It has been removed since it was causing issues and Prometheus now has native support for secret files, i.e. `basic_auth.password_file` and `authorization.credentials_file`."
    )
  ...
  options.services.prometheus = {
    ...
    user = mkOption {
      ...
      default = "_prometheus";
      description = ''
        User account under which Prometheus runs.
      '';
    };
    ...
    group = mkOption {
      ...
      default = "_prometheus";
      description = ''
        Group account under which Prometheus runs.
      '';
    };
    ...
    dataDir = mkOption {
      ...
      description = ''
        Data directory for Prometheus metrics data.

        ::: {.note}
        If left as the default value this directory will automatically be created before Prometheus
    ...
    enableReload = mkOption {
      ...
      description = ''
        ...
        (`darwin-rebuild switch`) that changes the prometheus
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      )
      {
        ...
        message = "Prometheus data directory should be in a system location like /opt, /usr/local, or /var. Current path: ${cfg.dataDir}";
      }
      {
        assertion = cfg.port >= 1024;
        message = "Prometheus port must be >= 1024 for non-root users. Current port: ${toString cfg.port}";
      }
    ];
    ...
    users.knownGroups = ["_prometheus"];
    users.knownUsers = ["_prometheus"];

    environment.systemPackages = [
      cfg.package
      # Add Prometheus service management scripts
      (pkgs.writeShellScriptBin "prometheus-start" ''
        echo "Starting Prometheus service..."
        sudo launchctl load /Library/LaunchDaemons/org.nixos.prometheus.plist 2>/dev/null || echo "Service already loaded"
        echo "Prometheus service started"
      '')
      (pkgs.writeShellScriptBin "prometheus-stop" ''
        echo "Stopping Prometheus service..."
        sudo launchctl unload /Library/LaunchDaemons/org.nixos.prometheus.plist 2>/dev/null || echo "Service not loaded"
        echo "Prometheus service stopped"
      '')
      (pkgs.writeShellScriptBin "prometheus-restart" ''
        echo "Restarting Prometheus service..."
        sudo launchctl unload /Library/LaunchDaemons/org.nixos.prometheus.plist 2>/dev/null || echo "Service not loaded"
        sleep 2
        sudo launchctl load /Library/LaunchDaemons/org.nixos.prometheus.plist
        echo "Prometheus service restarted"
      '')
      (pkgs.writeShellScriptBin "prometheus-status" ''
        if sudo launchctl list | grep -q "org.nixos.prometheus"; then
          PID=$(sudo launchctl list | grep "org.nixos.prometheus" | awk '{print $1}')
          if [[ "$PID" =~ ^[0-9]+$ ]]; then
            echo "Prometheus service is running (PID: $PID)"
            echo "Web UI: http://${cfg.listenAddress}:${toString cfg.port}"
          else
            echo "Prometheus service is loaded but not running (status: $PID)"
          fi
        else
          echo "Prometheus service is not loaded"
        fi
      '')
      (pkgs.writeShellScriptBin "prometheus-logs" ''
        echo "=== Prometheus Logs ==="
        if [[ -f "${cfg.dataDir}/prometheus.log" ]]; then
          tail -n 50 "${cfg.dataDir}/prometheus.log"
        else
          echo "Log file not found at ${cfg.dataDir}/prometheus.log"
        fi
      '')
    ];
    ...
    launchd.daemons.prometheus = let
      prometheusScript = pkgs.writeShellScript "prometheus-daemon" ''
        set -euo pipefail
        ...
        cleanup() {
          if [[ -n "''${PROMETHEUS_PID:-}" ]] && kill -0 "$PROMETHEUS_PID" 2>/dev/null; then
            log "Shutting down Prometheus (PID: $PROMETHEUS_PID)"
            kill "$PROMETHEUS_PID"
        ...
        if [[ ! -d "${cfg.dataDir}" ]]; then
          log "ERROR: Prometheus data directory ${cfg.dataDir} does not exist!"
          exit 1
        fi

        # Start Prometheus in background
        log "Starting Prometheus daemon..."
        log "Command: ${cfg.package}/bin/prometheus ${concatStringsSep " " cmdlineArgs}"
        ${cfg.package}/bin/prometheus ${concatStringsSep " " cmdlineArgs} &
        PROMETHEUS_PID=$!
        log "Prometheus daemon started with PID: $PROMETHEUS_PID"

        # Wait for Prometheus to be ready
        log "Waiting for Prometheus to be ready..."
        TIMEOUT=30
        COUNTER=0
        while ! curl -s "http://${cfg.listenAddress}:${toString cfg.port}/-/ready" > /dev/null 2>&1; do
          if ! kill -0 $PROMETHEUS_PID 2>/dev/null; then
            log "ERROR: Prometheus daemon (PID: $PROMETHEUS_PID) exited unexpectedly"
            exit 1
          fi
          if [ $COUNTER -ge $TIMEOUT ]; then
            log "ERROR: Prometheus failed to start within $TIMEOUT seconds"
            exit 1
          fi
          sleep 1
          COUNTER=$((COUNTER + 1))
        done
        log "Prometheus is ready at http://${cfg.listenAddress}:${toString cfg.port}"

        # Wait for Prometheus daemon to finish
        log "Prometheus service is running. Waiting for shutdown signal..."
        wait $PROMETHEUS_PID
      '';
    in {
      ...
      serviceConfig = {
        ...
        StandardOutPath = "${cfg.dataDir}/prometheus.log";
        StandardErrorPath = "${cfg.dataDir}/prometheus.error.log";
        UserName = cfg.user;
        GroupName = cfg.group;
