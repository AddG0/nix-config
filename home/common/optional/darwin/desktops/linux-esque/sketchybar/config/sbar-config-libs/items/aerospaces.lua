...
local Promise = require("sbar-config-libs/promise")
...
local workspace_order = { "Code", "Browser", "Ghostty", "Slack" }
local function workspace_sort_key(name)
  for i, v in ipairs(workspace_order) do
    if v == name then return string.format("%04d", i) end
  end
  return "9999" .. name
end
...
local function initialize()
  ...
  getAllWorkspaces():thenCall(function(workspaces)
    table.sort(workspaces, function(a, b)
      return workspace_sort_key(a["workspace"]) < workspace_sort_key(b["workspace"])
    end)
