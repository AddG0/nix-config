...
# while SDL freezes-until-resize on Hyprland+NVIDIA. To restore, prefix each
# branch's exec below with:  env -u WAYLAND_DISPLAY
{
  lib,
  pkgs,
  hyprctl,
  ...
  # gamescope's own Wayland server advertises neither wp_viewporter nor
  # wl_subcompositor, so a nested instance dies on "xdg_backend: Couldn't
  # create Wayland objects". The session's instance already does this job.
  passThroughIfNested = ''
    [ -z "''${GAMESCOPE_WAYLAND_DISPLAY:-}" ] || exec "$@"
  '';
  inherit (primaryMonitor) width height refreshRate;
  ...
  launcher =
    ...
    in
      ...
      pkgs.writeShellApplication {
        name = "gamescope-primary";
        runtimeInputs = [pkgs.gamescope];
        text = ''
          ${passThroughIfNested}
          exec gamescope \
            -W ${toString width} -H ${toString height} \
            -w ${toString width} -h ${toString height} -r ${toString refreshRate} -f \
