...
# First, resize the image to target dimensions (preserve transparency)
magick "$INPUT" -background none -resize "${TARGET_SIZE}x${TARGET_SIZE}^" -gravity center -extent "${TARGET_SIZE}x${TARGET_SIZE}" "$TEMP_FILE"
...
if [[ $EXT_LOWER == "png" ]]; then
	...
	magick "$INPUT" -background none -resize "${TARGET_SIZE}x${TARGET_SIZE}^" -gravity center -extent "${TARGET_SIZE}x${TARGET_SIZE}" \
